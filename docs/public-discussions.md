...
The adapter is independently usable and is also wired into `generateKit`. Search failures or unresolved identity produce explicit outcomes and warnings while the pipeline continues from the JD and any official evidence.

## Company identity and outbound data

`resolveCompanyIdentity` requires textual evidence. It can recognize an explicit `Company: Name` JD line, a same-origin page title segment corroborated by a hostname label, or an evidence-bearing candidate supplied by the caller. Candidate quotes must occur in the supplied JD or same-origin page and contain the name. The resolver never invents a name from a URL alone, and conflicting names cause a skip. Textual support does not by itself prove that an arbitrary mention identifies the employer, so callers remain responsible for candidate semantics.
...
Anecdotes may be outdated or wrong and are never official company policy. Embedded instructions remain untrusted source text. The pipeline cites only items actually used and does not count search attempts or robots files as evidence.
...
Search defaults are 30 seconds overall at the pipeline boundary, eight seconds per operation, 10 total HTTP attempts (including robots/retries), one retry, and the existing 1 MiB encoded/decoded response limit. Provider redirects are restricted to the configured origin and API/robots paths. Caller source restrictions are combined with that restriction. Options, endpoint overrides, resolver/transport injection, and local fixture policies are trusted process/test configuration and are never exposed as request parameters. Pacing is enforced per retrieval client, while the provider gate coordinates model traffic across active jobs in the process. These limits fit within the pipeline deadline.

## Verification

Verification covers successful, empty, partial, failed, blocked, malformed, ambiguous-identity, common-name, redirect, and NAT64 behavior through network-free and real loopback HTTP tests.
...
The fetcher recognizes only the well-known NAT64 `64:ff9b::/96` prefix and permits it only when its embedded IPv4 passes the public-unicast classification. Restricted embedded addresses remain blocked, including under local fixture policy. All DNS answers are checked, and the validated original IPv6 address remains pinned. Network-specific NAT64 and other classified transition ranges are not enabled. Deprecated IPv4-compatible `::/96` addresses classified as unicast by the dependency are explicitly rejected.

To repeat the public smoke check after building core:
...
This query verifies live search and filtering only. Full generation performance is documented in [live-benchmark.md](live-benchmark.md).
